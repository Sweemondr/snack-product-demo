...
function AppsView({ go }: { go: Navigate }) {
  return <div className="pd-apps-wrap"><PageHeading title="应用" description="连接工具，把 Snack 的工作能力扩展到更多真实场景。" /><div className="pd-app-feature"><button className="pd-record-settings-entry" aria-label="Snack Record 设置" title="录音设置" onClick={() => go("record-settings")}><SIcon name="settings" size={20} /></button><div className="pd-record-brand"><span><SIcon name="mic" size={28} /></span><div><em>SNACK 原生应用</em><h2>Snack Record</h2><p>本地会议录音与转写。结束后，Snack 会一边生成转译与纪要，一边通过对话确认后续跟踪。</p><div className="pd-tags"><span>本地录音</span><span>离线转写</span><span>对话式跟踪</span></div></div></div><div className="pd-record-actions"><button className="pd-record-start" aria-label="开始录音" title="开始录音" onClick={() => go("record")}><SIcon name="mic" size={21} /></button><button className="pd-record-library-entry" onClick={() => go("record-library")}><SIcon name="music" size={17} />我的录音</button></div></div><div className="pd-section-row"><div><h2>已连接应用</h2><p>在会话和智能体工作中直接使用</p></div><button className="pd-outline"><SIcon name="plus" size={16} />添加应用</button></div><div className="pd-app-grid">{[["飞书", "连接文档、日历、任务和团队消息", "飞", "#3370ff", "已连接"], ["Figma", "读取设计稿并参与产品设计协作", "F", "#a259ff", "已连接"], ["GitHub", "查看代码、Issue、PR 和构建状态", "G", "#0f172a", "连接"], ["语雀", "读取团队知识库与项目文档", "语", "#00b96b", "连接"]].map(([name, desc, icon, color, state]) => <article key={name}><header><span style={{ background: `${color}14`, color }}>{icon}</span><em className={state === "已连接" ? "connected" : ""}>{state === "已连接" && <i />}{state}</em></header><h3>{name}</h3><p>{desc}</p><footer><span>团队可用</span><button><SIcon name="settings" size={16} />设置</button></footer></article>)}</div></div>;
}
...
function RecordSettings({ go }: { go: Navigate }) {
  ...
  const [firstTime] = useState(() => typeof window !== "undefined" && (new URLSearchParams(window.location.search).get("setup") === "1" || window.localStorage.getItem("snack-record-configured-v1") !== "true"));
  const saveSettings = () => {
    window.localStorage.setItem("snack-record-configured-v1", "true");
    setSaved(true);
    if (firstTime) window.setTimeout(() => go("apps"), 650);
    else window.setTimeout(() => setSaved(false), 1600);
  };
  return <div className="pd-record-page settings"><RecordHeader title={firstTime ? "首次配置 Snack Record" : "Snack Record 设置"} desc={firstTime ? "完成一次基础配置，之后可从应用卡片右上角随时修改" : "配置本地录音、转写和文件保存方式"} back={() => go("apps")} /><div className="pd-settings-content">{firstTime && <section className="pd-settings-welcome"><span><SIcon name="mic" size={24} /></span><div><em>首次使用</em><h2>先确认录音与转写的默认方式</h2><p>只需要设置一次。完成后，首页卡片会直接显示“开始录音”和“我的录音”。</p></div><small>1 / 1</small></section>}<section className="pd-settings-card"><header><h2>默认配置</h2><p>以下设置会应用到新的录音任务，之后也可以随时调整</p></header><div className="pd-setting-row"><div><b>界面语言</b><p>设置 Snack Record 的默认界面语言</p></div><select aria-label="界面语言"><option>简体中文</option><option>English</option></select></div><div className="pd-setting-row"><div><b>转写模式</b><p>快速模式更省资源，标准模式准确率更高</p></div><div className="pd-segment"><button className={fast ? "active" : ""} onClick={() => setFast(true)}>快速</button><button className={!fast ? "active" : ""} onClick={() => setFast(false)}>标准</button></div></div><SettingToggle title="自动会议提醒" desc="检测到会议应用时提醒开始录音" checked={reminder} set={setReminder} /><div className="pd-setting-row"><div><b>录音快捷键</b><p>在任意界面快速开始或结束录音</p></div><kbd>⌃ R</kbd></div><div className="pd-setting-row"><div><b>输出目录</b><p>录音和转写文件将保存在本地</p></div><div className="pd-directory">~/Documents/Snack Record <button><SIcon name="folder" size={15} />选择</button></div></div><SettingToggle title="按日期整理文件" desc="自动创建年月文件夹保存录音" checked={organize} set={setOrganize} /><footer><button className="pd-outline"><SIcon name="refresh" size={16} />恢复默认</button><button className="pd-primary" onClick={saveSettings}>{saved ? <><SIcon name="check" size={16} />{firstTime ? "配置完成" : "已保存"}</> : firstTime ? "完成配置并进入应用" : "保存配置"}</button></footer></section><section className="pd-resource-card"><span><SIcon name="download" /></span><div><h2>本地转写资源包</h2><p>Whisper Standard · 1.42 GB · 资源完整</p></div><em><SIcon name="check" size={15} />可用</em><button className="pd-outline">检查完整性</button></section></div></div>;
}
...
function ConversationBubble({ children, user = false }: { children: React.ReactNode; user?: boolean }) {
  return <div className={`pd-conversation-row ${user ? "user" : "assistant"}`}>{!user && <span className="pd-conversation-avatar"><SIcon name="sparkle" size={16} /></span>}<div className="pd-conversation-bubble">{children}</div>{user && <Avatar text="高" />}</div>;
}

const followupTasks = [
  ["完成 onboarding 引导稿", "陆铭", "8 月 5 日"],
  ["补齐激活率口径与历史基线", "林可", "8 月 6 日"],
  ["复盘实验转化和次日留存", "高翔", "8 月 8 日"],
];

function RecordSummary({ go }: { go: Navigate }) {
  const [stage, setStage] = useState(0);
  const [projectName, setProjectName] = useState("AI 营销增长系统");
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const threadRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const timer = window.setTimeout(() => setStage(1), 1050);
    return () => window.clearTimeout(timer);
  }, []);
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => threadRef.current?.scrollTo({ top: threadRef.current.scrollHeight, behavior: stage > 1 ? "smooth" : "auto" }));
    return () => window.cancelAnimationFrame(frame);
  }, [stage]);
  const answer = (key: string, value: string, next: number) => {
    setAnswers((old) => ({ ...old, [key]: value }));
    setStage(next);
  };
  const tracked = Boolean(answers.project);
  return <div className="pd-record-page summary conversation"><RecordHeader title="录音已结束" desc="转译与会议纪要正在生成，Snack 会通过对话继续确认跟踪设置" back={() => go("record-library")} /><div className="pd-conversation-layout"><section className="pd-conversation-main"><header><div><span className="pd-live-finished"><SIcon name="check" size={15} /></span><div><h2>产品周会 · 智能协作</h2><p>原始录音 32:18 · 6 位参与者 · 刚刚结束</p></div></div><em><i />自动保存到我的录音</em></header><div className="pd-conversation-thread" ref={threadRef}><ConversationBubble><p>录音已保存。我正在转译完整内容并生成会议纪要；你不需要等待，我会同时用几个问题帮你把后续工作接住。</p><div className="pd-processing-card"><div><span><SIcon name="file" size={16} /></span><p><b>完整转译</b><small>{stage === 0 ? "正在识别发言人和时间点…" : "4,286 字 · 已完成"}</small></p><em className={stage === 0 ? "working" : "done"}>{stage === 0 ? "82%" : <><SIcon name="check" size={13} />完成</>}</em></div><div><span><SIcon name="sparkle" size={16} /></span><p><b>会议纪要</b><small>{stage === 0 ? "正在提取结论、行动项和指标…" : "1 项决策 · 3 个行动项 · 2 个指标"}</small></p><em className={stage === 0 ? "working" : "done"}>{stage === 0 ? "生成中" : <><SIcon name="check" size={13} />完成</>}</em></div></div></ConversationBubble>{stage >= 1 && <ConversationBubble><p>纪要已经生成：本周先以 20% 流量验证新版 onboarding，引导首日关键路径完成率提升到 65%。</p><div className="pd-inline-minutes"><span><SIcon name="sparkle" size={18} /></span><div><small>会议纪要</small><b>产品周会 · 智能协作</b><p>已保留完整转译和时间点，可随时回听原始录音。</p></div><button>查看纪要<SIcon name="chevron" size={14} /></button></div><p className="pd-conversation-question">这些事项跨越多天且需要多人协作，需要继续跟踪吗？</p>{stage === 1 && <div className="pd-quick-actions"><button className="primary" onClick={() => answer("track", "创建项目继续跟踪", 2)}><SIcon name="folder" size={16} />创建项目继续跟踪</button><button onClick={() => answer("track", "只保存纪要", 7)}>只保存纪要</button></div>}</ConversationBubble>}{answers.track && <ConversationBubble user><p>{answers.track}</p></ConversationBubble>}{stage >= 2 && stage < 7 && <ConversationBubble><p>我已经根据纪要补好了项目草稿。请确认项目上下文和成员；这些都可以稍后调整。</p><div className="pd-chat-project-card"><label>项目名称<input value={projectName} onChange={(event) => setProjectName(event.target.value)} /></label><div className="pd-chat-project-row"><span><SIcon name="link" size={15} />项目上下文</span><b>增长实验 Wiki</b><b>本地 /Growth/Onboarding</b></div><div className="pd-chat-project-row"><span><SIcon name="users" size={15} />项目成员</span><span className="pd-stacked big"><Avatar small text="高" /><Avatar small text="陆" color="#5b8def" /><Avatar small text="林" color="#18a87b" /><Avatar small text="周" color="#a46be8" /></span><b>4 人</b></div></div>{stage === 2 && <div className="pd-quick-actions"><button className="primary" onClick={() => answer("project", projectName, 3)}><SIcon name="check" size={16} />确认创建项目</button><button onClick={() => answer("track", "暂不创建项目，只保存纪要", 7)}>暂不创建</button></div>}</ConversationBubble>}{answers.project && <ConversationBubble user><p>确认创建“{answers.project}”</p></ConversationBubble>}{tracked && stage >= 3 && <ConversationBubble><p>要同时创建项目群聊吗？纪要、任务变化和会前简报可以自动同步到群聊。</p>{stage === 3 && <div className="pd-quick-actions"><button className="primary" onClick={() => answer("group", "创建项目群聊", 4)}><SIcon name="users" size={16} />创建群聊</button><button onClick={() => answer("group", "暂不创建群聊", 4)}>暂不创建</button></div>}</ConversationBubble>}{answers.group && <ConversationBubble user><p>{answers.group}</p></ConversationBubble>}{tracked && stage >= 4 && <ConversationBubble><p>我从纪要里提取了 3 个行动项，负责人和截止时间也已补齐。要写入项目 Task Hub 吗？</p><div className="pd-chat-task-list">{followupTasks.map(([task, owner, date], index) => <div key={task}><span>{index + 1}</span><p><b>{task}</b><small>{owner} · {date}</small></p><SIcon name="check" size={15} /></div>)}</div>{stage === 4 && <div className="pd-quick-actions"><button className="primary" onClick={() => answer("tasks", "将 3 个任务写入 Task Hub", 5)}><SIcon name="task" size={16} />写入 3 个任务</button><button onClick={() => answer("tasks", "只保留在纪要中", 5)}>只保留在纪要中</button></div>}</ConversationBubble>}{answers.tasks && <ConversationBubble user><p>{answers.tasks}</p></ConversationBubble>}{tracked && stage >= 5 && <ConversationBubble><p>这次会议明确了核心目标和两个关注指标。是否加入项目快照持续跟踪？</p><div className="pd-chat-metrics"><div><span><SIcon name="target" size={17} /></span><p><small>核心目标</small><b>首日关键路径完成率</b></p><strong>65%</strong></div><div><span>激活率 <b>42.8%</b></span><span>次日留存 <b>31.4%</b></span></div></div>{stage === 5 && <div className="pd-quick-actions"><button className="primary" onClick={() => answer("metrics", "持续跟踪目标与指标", 6)}><SIcon name="target" size={16} />持续跟踪</button><button onClick={() => answer("metrics", "暂不跟踪指标", 6)}>暂不跟踪</button></div>}</ConversationBubble>}{answers.metrics && <ConversationBubble user><p>{answers.metrics}</p></ConversationBubble>}{tracked && stage >= 6 && <ConversationBubble><p>最后一项：下次产品周会是 8 月 8 日 16:00。需要会前提醒，并自动发送项目简报吗？</p><div className="pd-chat-reminder"><span><SIcon name="calendar" size={18} /></span><p><b>产品周会 · 智能协作</b><small>会前 1 天提醒 · 汇总任务进展、指标变化和最新资料</small></p></div>{stage === 6 && <div className="pd-quick-actions"><button className="primary" onClick={() => answer("reminder", "开启会前提醒与简报", 7)}><SIcon name="bell" size={16} />开启提醒与简报</button><button onClick={() => answer("reminder", "暂不开启提醒", 7)}>暂不开启</button></div>}</ConversationBubble>}{answers.reminder && <ConversationBubble user><p>{answers.reminder}</p></ConversationBubble>}{stage === 7 && <ConversationBubble><div className="pd-chat-complete"><span><SIcon name="check" size={22} /></span><div><small>{tracked ? "后续工作已接住" : "会议资料已保存"}</small><h3>{tracked ? `${answers.project} 已就绪` : "录音、转译与会议纪要已归档"}</h3><p>{tracked ? `已按你的选择完成配置：${answers.group || "未创建群聊"}，${answers.tasks || "任务未写入"}，${answers.metrics || "指标未跟踪"}，${answers.reminder || "提醒未开启"}。` : "没有强制创建项目；之后可以从“我的录音”再次发起跟踪。"}</p></div></div><div className="pd-quick-actions">{tracked && <button className="primary" onClick={() => go("project")}>进入项目主页<SIcon name="arrow" size={16} /></button>}<button onClick={() => go("record-library")}>返回我的录音</button></div></ConversationBubble>}</div></section><aside className="pd-conversation-aside"><header><span><SIcon name="sparkle" size={16} /></span><div><b>本次会议产物</b><small>随对话实时更新</small></div></header><div className="pd-artifact-item complete"><span><SIcon name="music" size={16} /></span><p><b>原始录音</b><small>32:18 · 已保存</small></p><SIcon name="check" size={14} /></div><div className={`pd-artifact-item ${stage >= 1 ? "complete" : "working"}`}><span><SIcon name="file" size={16} /></span><p><b>完整转译</b><small>{stage >= 1 ? "4,286 字 · 已完成" : "识别中 82%"}</small></p>{stage >= 1 ? <SIcon name="check" size={14} /> : <i />}</div><div className={`pd-artifact-item ${stage >= 1 ? "complete" : "working"}`}><span><SIcon name="sparkle" size={16} /></span><p><b>会议纪要</b><small>{stage >= 1 ? "决策与行动项已生成" : "正在生成"}</small></p>{stage >= 1 ? <SIcon name="check" size={14} /> : <i />}</div><div className={`pd-artifact-item ${tracked ? "complete" : "pending"}`}><span><SIcon name="folder" size={16} /></span><p><b>跟踪项目</b><small>{tracked ? answers.project : "等待你的选择"}</small></p>{tracked ? <SIcon name="check" size={14} /> : <em>可选</em>}</div><div className={`pd-artifact-item ${answers.tasks?.startsWith("将") ? "complete" : "pending"}`}><span><SIcon name="task" size={16} /></span><p><b>跟踪任务</b><small>{answers.tasks || "3 项待确认"}</small></p>{answers.tasks?.startsWith("将") ? <SIcon name="check" size={14} /> : <em>可选</em>}</div><div className={`pd-artifact-item ${answers.reminder?.startsWith("开启") ? "complete" : "pending"}`}><span><SIcon name="bell" size={16} /></span><p><b>会前简报</b><small>{answers.reminder || "等待确认"}</small></p>{answers.reminder?.startsWith("开启") ? <SIcon name="check" size={14} /> : <em>可选</em>}</div><footer><SIcon name="check" size={14} /><span>每一步都可跳过，稍后也能修改</span></footer></aside></div></div>;
}
