...
const state = {
  ...
  agentTab: 'square',
  resourceTab: 'all',
  localKnowledgeModalOpen: false,
  localKnowledgeFiles: [],
  localKnowledgeSource: null,
  localKnowledgeProjectId: null,
  localKnowledgeAssetId: null,
  localKnowledgeSyncToCloud: true,
  ...
  projectMemberPickerType: 'human',
  projectMemberEditorOpen: false,
  projectEditingId: null,
  projectBoardTab: 'tasks',
  projectDetailTab: 'collaborators',
  projectGroupChatOpen: false,
  projectGroupChatProjectId: null,
  projectGroupChatQuery: '',
  projectGroupChatFilter: 'all',
  projectGroupChatSelection: [],
  projectGroupChatEditingGroupId: null,
  projectGroupPage: 1,
  projectGroupActionModal: null,
  projectGroupMembersModal: null,
  projectCollaboratorManagerType: null,
  projectCollaboratorPickerType: null,
  projectCollaboratorQuery: '',
  projectCollaboratorPermissionMember: null,
...
function isSnackDesktopRuntime() {
  ...
  return /(?:^|\s)SnackDesktop\//i.test(userAgent)
    || document.documentElement.dataset.snackHost === 'snack-desktop';
}
...
const projectWikiTopicOptions = [
  {
    name: 'Snack 产品空间',
    description: '语雀中的产品规划、需求与设计文档',
    source: '语雀空间',
    type: 'yuque',
  },
  {
    name: '市场运营空间',
    description: '语雀中的市场活动与运营协作资料',
    source: '语雀空间',
    type: 'yuque',
  },
  {
    name: '即服务知识库',
    description: '已构建完成的服务方案与客户交付知识',
    source: 'LLM Wiki',
    type: 'llm-wiki',
  },
  {
    name: '平台业务知识库',
    description: '已构建完成的平台规划与业务知识',
    source: 'LLM Wiki',
    type: 'llm-wiki',
  },
];
...
const projectFolders = [
  {
    ...
    groups: [
      ...
      {
        id: 'snack-product-weekly-group',
        name: '产品周会协作群',
        createdAt: '2026年7月25日',
        participants: ['田晓柔', 'Snack', '产品设计 Agent', '研发交付 Agent'],
      },
      {
        id: 'snack-taskhub-review-group',
        name: 'Task Hub 评审群',
        createdAt: '2026年7月28日',
        participants: ['田晓柔', 'Snack', '前端工程师-小贝', 'QA-小林'],
      },
      {
        id: 'snack-record-experience-group',
        name: 'Snack Record 体验群',
        createdAt: '2026年7月30日',
        participants: ['田晓柔', 'Snack', '产品设计 Agent', 'QA-小林'],
      },
      {
        id: 'snack-mobile-integration-group',
        name: '移动端联调群',
        createdAt: '2026年7月31日',
        participants: ['田晓柔', 'Snack', '前端工程师-小贝'],
      },
      {
        id: 'snack-user-research-group',
        name: '用户研究同步群',
        createdAt: '2026年8月1日',
        participants: ['田晓柔', 'Snack', '产品设计 Agent'],
      },
      {
        id: 'snack-release-prep-group',
        name: '版本发布准备群',
        createdAt: '2026年8月3日',
...
function render() {
  ...
  mainSurface.classList.toggle('agent-workspace-surface', state.view === 'agents');
  mainSurface.innerHTML = `${renderMainView()}${renderLocalKnowledgePickerInputs()}${state.projectCreationOpen ? renderProjectCreationModal() : ''}${state.issueCreationOpen ? renderIssueCreationModal() : ''}${state.projectGroupChatOpen ? renderProjectGroupChatModal() : ''}${state.projectGroupMembersModal ? renderProjectGroupMembersModal() : ''}${state.projectGroupActionModal ? renderProjectGroupActionModal() : ''}${renderLocalKnowledgeModal()}${renderProjectSessionImportModal()}${renderMeetingOverlays()}${renderSnackRecordOverlays()}`;
  renderIcons();
...
function getSecondaryTabs() {
  if (['tasks', 'issue', 'projectSchedule'].includes(state.view)) {
    return [
...
function renderSecondaryTabs() {
  ...
  topbar.classList.toggle('agent-workspace-topbar', state.view === 'agents');
...
function syncTopbarVisibility() {
  topbar.hidden = ['project', 'projectBoard', 'recordSummary'].includes(state.view);
}

function renderTopbarActions() {
  const canCreateIssue = ['tasks', 'issue'].includes(state.view);
  ...
  return `
    <section class="history-project conversation-drop-zone ${expanded ? 'expanded' : ''} ${project.pinnedAt ? 'pinned-project' : ''}" data-conversation-drop-project="${project.id}">
      <div class="history-project-row">
        ...
        ` : `
          <button class="history-project-title ${activeFolder ? 'active' : ''}" data-project-toggle="${project.id}" aria-expanded="${expanded ? 'true' : 'false'}">
            <i data-lucide="${expanded ? 'folder-open' : 'folder'}"></i>
        ...
        <span class="history-project-create-wrap">
          <button class="history-add" aria-label="新建会话" data-project-create-action="single" data-project-id="${project.id}">
            <i data-lucide="plus"></i>
          </button>
        </span>
...
function renderProjectMoreMenu(project) {
  ...
  return `
    <div class="project-more-menu" role="menu" aria-label="${escapeAttribute(project.title)} 更多操作">
      <button class="project-menu-item" data-project-action="add-group" data-project-id="${project.id}">
...
function renderProjectSessions(project) {
  ...
  return `
    <button class="history-session project-board-session ${boardActive ? 'active' : ''}" data-project-open="${escapeAttribute(project.id)}">
      <i data-lucide="kanban-square"></i>
      <span>项目看板</span>
    </button>
    ${allSessions.map((session) => renderHistorySession(project, session)).join('')}
  `;
}
...
function renderProjectBoardPage(project) {
  ...
  const projectTaskBoard = `
    <section class="project-task-workbench" aria-label="${escapeAttribute(project.title)}任务看板">
      <header class="project-task-toolbar">
        ${renderTaskFilterBar(projectIssues)}
  ...
  `;
  const projectTabContent = state.projectBoardTab === 'todos'
    ? renderProjectWorkList(projectTodos, 'todos')
    : state.projectBoardTab === 'collaborators'
      ? renderProjectCollaboratorsPage(project)
      : state.projectBoardTab === 'knowledge'
        ? renderProjectKnowledgePage(project)
        : projectTaskBoard;
  const boardContent = `
    <section class="project-board-shell">
      <section class="project-board-overview">
        <section class="project-overview-header">
          <div>
            <span class="project-overview-title-row">
              <h1>${escapeHtml(project.title)}</h1>
              <button class="project-overview-edit" type="button" data-project-config="${escapeAttribute(project.id)}">
                <i data-lucide="pencil"></i>
                <span>编辑</span>
              </button>
            </span>
        ...
        <nav class="project-work-tabs" aria-label="项目工作内容">
          <button class="${state.projectBoardTab === 'tasks' ? 'active' : ''}" type="button" data-project-board-tab="tasks">
            <span>任务</span><em>${projectIssues.length}</em>
          </button>
          <button class="${state.projectBoardTab === 'todos' ? 'active' : ''}" type="button" data-project-board-tab="todos">
      ...
      <section class="project-board-content">
...
function renderProjectWorkList(projectIssues, tab) {
  const isTodo = tab === 'todos';
  return `
    <section class="project-work-area">
      <header>
        <div>
          <strong>${isTodo ? '项目待办' : '项目任务'}</strong>
          <span>${isTodo ? '需要你确认或继续推进的事项' : '项目中的全部执行任务'}</span>
        </div>
        ${isTodo ? '' : '<button type="button" data-create-issue><i data-lucide="plus"></i><span>创建任务</span></button>'}
      </header>
      ${projectIssues.length ? `
        <div class="project-work-list">
          ${projectIssues.map((issue) => `
            <button type="button" data-issue-id="${escapeAttribute(issue.code)}">
              <span class="project-work-status"><i data-lucide="${isTodo ? 'circle-check-big' : 'clipboard-list'}"></i></span>
              <span class="project-work-copy">
                <strong>${escapeHtml(issue.title)}</strong>
                <small>${escapeHtml(issue.code)} · ${escapeHtml(issue.owner || '待分配')}</small>
              </span>
              <em>${escapeHtml(getIssueStateLabel(issue))}</em>
              <i data-lucide="chevron-right"></i>
            </button>
          `).join('')}
        </div>
      ` : `
        <div class="project-work-empty">
          <span><i data-lucide="${isTodo ? 'circle-check-big' : 'clipboard-list'}"></i></span>
          <strong>${isTodo ? '暂无项目待办' : '暂无项目任务'}</strong>
          <p>${isTodo ? '需要确认或推进的事项会出现在这里。' : '创建第一条任务，开始推进项目。'}</p>
        </div>
      `}
    </section>
  `;
...
function renderProjectCollaboratorsPage(project) {
...
function renderProjectGroupList(project, groups) {
  ...
  const totalPages = Math.max(1, Math.ceil(groups.length / projectGroupPageSize));
  const currentPage = Math.min(Math.max(1, state.projectGroupPage), totalPages);
  const pageStart = (currentPage - 1) * projectGroupPageSize;
  const visibleGroups = groups.slice(pageStart, pageStart + projectGroupPageSize);
  return `
    ...
    return `
      ...
      <nav class="project-group-pagination" aria-label="群聊列表分页">
        <button type="button" data-project-group-page="${currentPage - 1}" ${currentPage === 1 ? 'disabled' : ''} aria-label="上一页"><i data-lucide="chevron-left"></i></button>
        ${Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => `
          <button class="${page === currentPage ? 'active' : ''}" type="button" data-project-group-page="${page}" aria-current="${page === currentPage ? 'page' : 'false'}">${page}</button>
        `).join('')}
        <button type="button" data-project-group-page="${currentPage + 1}" ${currentPage === totalPages ? 'disabled' : ''} aria-label="下一页"><i data-lucide="chevron-right"></i></button>
      </nav>
    </div>
  `;
}

function setProjectGroupPage(page) {
  const project = getActiveProject();
  const totalPages = Math.max(1, Math.ceil(getProjectGroups(project || {}).length / projectGroupPageSize));
  ...
  return `
    <section class="project-people-section ${managing ? 'managing' : ''}">
      ...
      <div class="project-card-member-list">
        ${members.length
    ? members.map((member) => renderProjectCollaboratorNameRow(project, member, managing)).join('')
...
function getProjectMemberPermission(project, name) {
  if (name === currentUserName) return 'manage';
  return project.memberPermissions?.[name] === 'manage' ? 'manage' : 'use';
}

function getProjectMemberPermissionLabel(permission) {
  return permission === 'manage' ? '可管理' : '可使用';
}

function renderProjectCollaboratorNameRow(project, member, managing) {
  const canRemove = managing && member.name !== currentUserName && member.name !== 'Snack';
  const permission = member.isAgent ? null : getProjectMemberPermission(project, member.name);
  const permissionOpen = state.projectCollaboratorPermissionMember === member.name;
  const permissionEditable = Boolean(permission && managing && member.name !== currentUserName);
  return `
    <article class="project-collaborator-name-row">
      <span class="group-member-avatar ${member.isAgent ? 'agent' : 'human'}">${escapeHtml(member.name.slice(0, 1))}</span>
      <strong>${escapeHtml(member.name)}</strong>
      ${permission ? `
        <span class="project-collaborator-permission ${permissionEditable ? 'editable' : ''} ${permissionOpen ? 'open' : ''}">
          ${permissionEditable ? `
            <button type="button" data-project-collaborator-permission="${escapeAttribute(member.name)}" aria-haspopup="menu" aria-expanded="${permissionOpen}">
              <span>${getProjectMemberPermissionLabel(permission)}</span><i data-lucide="chevron-down"></i>
            </button>
            ${permissionOpen ? `
              <span class="project-collaborator-permission-menu" role="menu" aria-label="修改 ${escapeAttribute(member.name)} 的项目权限">
                <button class="${permission === 'manage' ? 'selected' : ''}" type="button" role="menuitem" data-project-collaborator-permission-value="manage" data-project-collaborator-permission-member="${escapeAttribute(member.name)}">
                  <i data-lucide="${permission === 'manage' ? 'circle-check-big' : 'shield-check'}"></i>
                  <span><strong>可管理</strong><small>可编辑项目配置和管理成员</small></span>
                </button>
                <button class="${permission === 'use' ? 'selected' : ''}" type="button" role="menuitem" data-project-collaborator-permission-value="use" data-project-collaborator-permission-member="${escapeAttribute(member.name)}">
                  <i data-lucide="${permission === 'use' ? 'circle-check-big' : 'eye'}"></i>
                  <span><strong>可使用</strong><small>可访问项目内容并参与协作</small></span>
                </button>
              </span>
            ` : ''}
          ` : `<span class="project-collaborator-permission-label">${getProjectMemberPermissionLabel(permission)}</span>`}
        </span>
      ` : ''}
      ${canRemove ? `
        <button class="project-collaborator-remove" type="button" data-member-manager-remove="${escapeAttribute(member.name)}" aria-label="移除 ${escapeAttribute(member.name)}">
          <i data-lucide="x"></i>
        </button>
      ` : ''}
    </article>
...
function refreshProjectCollaboratorsPage(options = {}) {
  const project = getActiveProject();
  const page = document.querySelector('.project-collaborators-page');
  if (!project || !(page instanceof HTMLElement)) return false;
  page.outerHTML = renderProjectCollaboratorsPage(project);
  renderIcons();
  if (options.focusSearch) syncProjectCollaboratorSearchFocus();
  return true;
}

function renderProjectKnowledgeAttachPicker(project) {
  const mountedTopics = new Set(project.wikiTopics || []);
  const open = state.projectKnowledgeAttachProjectId === project.id;
  return `
    <span class="project-knowledge-attach-control ${open ? 'open' : ''}" data-project-knowledge-attach-picker>
      <button
        class="project-info-config-entry project-knowledge-action-button"
        type="button"
        data-project-knowledge-picker-toggle="${escapeAttribute(project.id)}"
        aria-haspopup="dialog"
        aria-expanded="${open ? 'true' : 'false'}"
      ><i data-lucide="plus"></i><span>添加知识</span></button>
      ${open ? `
        <section class="project-knowledge-topic-menu" role="dialog" aria-label="选择要挂载的 Topic">
          <header><strong>选择 Topic</strong><small>选择后立即挂载到当前项目</small></header>
          <div class="project-knowledge-topic-options" role="listbox" aria-label="可挂载的 Wiki Topic">
            ${projectWikiTopicOptions.map((topic) => {
    const mounted = mountedTopics.has(topic.name);
    return `
                <button
                  class="project-knowledge-topic-option ${mounted ? 'mounted' : ''}"
                  type="button"
                  role="option"
                  aria-selected="${mounted ? 'true' : 'false'}"
                  aria-label="${escapeAttribute(topic.name)}${mounted ? '，已挂载' : ''}"
                  data-project-knowledge-topic="${escapeAttribute(topic.name)}"
                  data-project-id="${escapeAttribute(project.id)}"
                  ${mounted ? 'disabled' : ''}
                >
                  <span class="project-knowledge-topic-icon ${escapeAttribute(topic.type)}"><i data-lucide="${topic.type === 'yuque' ? 'book-open-text' : 'database'}"></i></span>
                  <span class="project-knowledge-topic-copy"><strong>${escapeHtml(topic.name)}</strong><small><em>${escapeHtml(topic.source)}</em>${escapeHtml(topic.description)}</small></span>
                  <span class="project-knowledge-topic-status">${mounted ? '<i data-lucide="check"></i>已挂载' : '<i data-lucide="plus"></i>'}</span>
                </button>
              `;
  }).join('')}
          </div>
          <footer><span>没有合适的 Topic？</span><button type="button" data-project-wiki-topic-create aria-label="创建 Wiki Topic（模拟外链）" title="将在新页面打开（模拟）"><i data-lucide="external-link"></i>创建 Wiki Topic</button></footer>
        </section>
      ` : ''}
    </span>
  `;
...
function toggleProjectKnowledgeAttachPicker(projectId) {
  if (!getProjectById(projectId)) return;
  state.projectKnowledgeAttachProjectId = state.projectKnowledgeAttachProjectId === projectId ? null : projectId;
  render();
}

function attachProjectKnowledgeTopic(projectId, topicName) {
  const project = getProjectById(projectId);
  if (!project || !projectWikiTopicOptions.some((topic) => topic.name === topicName)) return;
  project.wikiTopics = [...new Set([...(project.wikiTopics || []), topicName])];
  state.projectKnowledgeAttachProjectId = null;
  render();
  showToast(`已挂载 Topic“${topicName}”`);
}

function simulateWikiTopicCreationLink() {
  state.projectKnowledgeAttachProjectId = null;
  render();
...
function attachProjectSessionAsset(projectId, sessionId) {
  ...
  const editingInProjectConfig = state.projectCreationOpen && state.projectEditingId === projectId;
  const currentAssets = editingInProjectConfig ? state.projectCreationSessionAssets : (project?.sessionAssets || []);
  if (!project || !session || currentAssets.some((item) => item.sourceSessionId === session.id)) return;
  const asset = {
  ...
  if (editingInProjectConfig) state.projectCreationSessionAssets = [...currentAssets, asset];
  else project.sessionAssets = [...currentAssets, asset];
  state.projectSessionImportProjectId = null;
  state.projectSessionImportQuery = '';
  render();
...
function renderProjectSessionImportModal() {
  ...
  const sessionAssets = state.projectCreationOpen && state.projectEditingId === project.id
    ? state.projectCreationSessionAssets
    : (project.sessionAssets || []);
  const attachedIds = new Set(sessionAssets.map((item) => item.sourceSessionId));
...
function getProjectWikiTopicAsset(projectId, assetId) {
  const project = getProjectById(projectId);
  const topicName = typeof assetId === 'string' && assetId.startsWith('wiki-topic:')
    ? assetId.slice('wiki-topic:'.length)
    : '';
  const topic = projectWikiTopicOptions.find((item) => item.name === topicName);
  if (!project || !topic || !(project.wikiTopics || []).includes(topicName)) return null;
  return {
    id: assetId,
    name: topic.name,
    kind: 'wiki-topic',
    topicName,
    source: topic.source,
    label: `${topic.source} · 云端读取`,
    sharedProjectIds: project.wikiTopicShares?.[topicName] || [],
  };
}
...
function renderProjectAssetVisibilityControl(project, asset) {
  ...
  const visibilityLabel = visibility === 'project' ? '项目成员可见' : '仅我可见';
  return `
...
function ensureProjectSourceAssets(project) {
  if (!project) return [];
  project.sourceFolders = (project.sourceFolders || []).map((folder, index) => {
    const asset = typeof folder === 'string' ? { name: folder } : folder;
    if (!asset.id) asset.id = `source-asset:${project.id}:${index}`;
    if (!asset.kind) asset.kind = 'local-files';
    return asset;
  });
  return project.sourceFolders;
}

function getProjectAssetListMeta(project, asset) {
  let type = '本地文件';
  if (asset.kind === 'wiki-topic') {
    type = String(asset.source || '').includes('语雀') ? '语雀' : 'LLM Wiki';
  } else if (asset.kind === 'session') {
    type = '会话';
  ...
  return {
    type,
    updated: asset.updated || project.updated || '刚刚',
    updatedBy: asset.updatedBy || currentUserName,
  };
}

function renderProjectAssetListRow(project, asset, selected) {
  const meta = getProjectAssetListMeta(project, asset);
  return `
    <article class="project-asset-table-row" role="row">
      <label class="project-asset-selection" role="cell">
        <input type="checkbox" data-project-asset-select data-project-id="${escapeAttribute(project.id)}" data-project-asset-id="${escapeAttribute(asset.id)}" aria-label="选择${escapeAttribute(asset.name)}" ${selected ? 'checked' : ''} />
      </label>
      <strong class="project-asset-name" role="cell" title="${escapeAttribute(asset.name)}">${escapeHtml(asset.name)}</strong>
      <span class="project-asset-type" role="cell">${escapeHtml(meta.type)}</span>
      <time role="cell">${escapeHtml(meta.updated)}</time>
      <span class="project-asset-updater" role="cell">${escapeHtml(meta.updatedBy)}</span>
      <div class="project-asset-permission" role="cell">
        ${renderProjectAssetVisibilityControl(project, asset)}
      </div>
      <div class="project-asset-row-actions" role="cell">
        <button type="button" data-project-asset-action="share" data-project-id="${escapeAttribute(project.id)}" data-project-asset-id="${escapeAttribute(asset.id)}"><i data-lucide="share-2"></i><span>分享</span></button>
        <button class="danger" type="button" data-project-asset-action="delete" data-project-id="${escapeAttribute(project.id)}" data-project-asset-id="${escapeAttribute(asset.id)}"><i data-lucide="trash-2"></i><span>删除</span></button>
      </div>
      ${renderProjectAssetShareDrawer(project, 'row', asset.id)}
    </article>
  `;
}
...
function renderProjectKnowledgePage(project) {
  ...
  const sourceFolders = ensureProjectSourceAssets(project);
  const assets = [
    ...wikiTopics.map((topicName) => getProjectWikiTopicAsset(project.id, getProjectWikiTopicAssetId(topicName))).filter(Boolean),
    ...sessionAssets,
    ...sourceFolders,
  ];
  const resourceCount = assets.length;
  const selectableAssetIds = assets.map((asset) => asset.id);
  const selectedAssetIds = getProjectAssetSelection(project.id);
  const allSelected = resourceCount > 0 && selectableAssetIds.every((assetId) => selectedAssetIds.includes(assetId));
  return `
    <section class="project-info-tab-page project-knowledge-page">
      ...
      <div class="project-asset-table" role="table" aria-label="项目资产列表">
        ${resourceCount ? `
          <div class="project-asset-table-head" role="row">
            <label class="project-asset-table-select-all" role="columnheader">
              <input type="checkbox" data-project-asset-select-all="${escapeAttribute(project.id)}" ${allSelected ? 'checked' : ''} />
              <span>全选</span>
            </label>
            <span role="columnheader">资产名称</span>
          ...
          ${assets.map((asset) => renderProjectAssetListRow(project, asset, selectedAssetIds.includes(asset.id))).join('')}
        ` : `
            <div class="project-knowledge-tab-empty">
              <span><i data-lucide="book-open-text"></i></span>
              <strong>尚未关联知识</strong>
              <p>点击“添加知识”挂载 Topic，或添加本地文件。</p>
            </div>
        `}
      </div>
    </section>
...
function renderProjectDetailPanel(project, options = {}) {
  ...
  return `
    <aside class="project-detail-panel ${inRecordSummary ? 'record-summary-project-detail' : ''}" id="${inRecordSummary ? 'recordSummaryProjectDetail' : 'projectBoardDetail'}" aria-label="${escapeAttribute(project.title)}协作信息">
      ${inRecordSummary ? `
        <header class="project-detail-header">
          <strong>项目详情</strong>
          <span>
            <button type="button" aria-label="关闭项目详情" title="关闭项目详情" data-record-action="close-summary-project-detail">
      ...
      ${inRecordSummary ? '' : `
        <nav class="project-detail-tabs" aria-label="项目协作信息">
          <button class="${detailTab === 'collaborators' ? 'active' : ''}" type="button" data-project-detail-tab="collaborators">
            <span>成员与智能体</span><em>${collaboratorCount}</em>
          </button>
          <button class="${detailTab === 'knowledge' ? 'active' : ''}" type="button" data-project-detail-tab="knowledge">
      ...
      <div class="project-detail-scroll">
        ${inRecordSummary || detailTab === 'collaborators' ? `
        <section class="project-detail-card project-detail-collaborators">
          <header>
            ${inRecordSummary
    ? '<span><i data-lucide="users-round"></i><strong>成员与智能体</strong></span>'
    : `<span class="project-detail-card-summary">共 ${collaboratorCount} 位协作对象</span>`}
            ${inRecordSummary ? `<em>${projectMembers.length + expertAgents.length}</em>` : `
              <button class="project-group-chat-entry" type="button" data-project-group-chat-open="${escapeAttribute(project.id)}">
                <i data-lucide="messages-square"></i>
                <span>发起群聊</span>
              </button>
            `}
          </header>
          <div class="project-detail-collaborator-groups">
            <section>
              <header><strong>项目成员</strong><em>${projectMembers.length}</em></header>
              <div class="project-detail-member-list">
                ${projectMembers.map((member) => renderProjectDetailMemberRow(member, { readOnly: true })).join('')}
              </div>
            </section>
            <section>
              <header><strong>专家智能体</strong><em>${expertAgents.length}</em></header>
              <div class="project-detail-member-list">
                ${expertAgents.length
    ...
    : '<p class="project-detail-empty">尚未添加专家智能体</p>'}
        ...
        ${inRecordSummary || detailTab === 'knowledge' ? `
        <section class="project-detail-card project-detail-knowledge">
          <header>
            ${inRecordSummary
    ? '<span><i data-lucide="paperclip"></i><strong>关联知识</strong></span>'
    : `<span class="project-detail-card-summary">${knowledgeCount ? `已关联 ${knowledgeCount} 项知识` : '尚未关联知识'}</span>`}
            ${inRecordSummary ? `<em>${knowledgeCount}</em>` : ''}
          </header>
...
function getProjectGroupChatDirectory(project) {
  const directory = new Map();
  const add = (member) => {
    if (!member?.name || directory.has(member.name)) return;
    directory.set(member.name, member);
  };
  add({ name: currentUserName, isAgent: false, role: '项目发起人' });
  add({ name: 'Snack', isAgent: true, role: '项目协作伙伴' });
  getProjectCollaborators(project).forEach(add);
  return [...directory.values()];
}

function getProjectGroupChatFilter() {
  return ['human', 'agent'].includes(state.projectGroupChatFilter)
    ? state.projectGroupChatFilter
    : 'all';
}

function getProjectGroupChatCandidates(project) {
  const query = state.projectGroupChatQuery.trim().toLowerCase();
  const filter = getProjectGroupChatFilter();
  return getProjectGroupChatDirectory(project)
    .filter((member) => member.name !== currentUserName && member.name !== 'Snack')
    .filter((member) => filter === 'all' || member.isAgent === (filter === 'agent'))
    .filter((member) => !query || `${member.name} ${member.role}`.toLowerCase().includes(query));
}

function renderProjectGroupChatFilters() {
  const filter = getProjectGroupChatFilter();
  return `
    <nav class="project-group-chat-filters" aria-label="群聊对象筛选">
      <button class="${filter === 'all' ? 'active' : ''}" type="button" data-project-group-chat-filter="all">全部</button>
      <button class="${filter === 'human' ? 'active' : ''}" type="button" data-project-group-chat-filter="human">项目成员</button>
      <button class="${filter === 'agent' ? 'active' : ''}" type="button" data-project-group-chat-filter="agent">专家智能体</button>
    </nav>
  `;
}

function renderProjectGroupChatOptions(project) {
  const selectedNames = new Set(state.projectGroupChatSelection);
  const candidates = getProjectGroupChatCandidates(project);
  return `
    <div class="project-group-chat-options" role="listbox" aria-label="可加入群聊的对象" aria-multiselectable="true">
      ${candidates.length ? candidates.map((member) => {
    const selected = selectedNames.has(member.name);
    return `
          <button class="${selected ? 'selected' : ''}" type="button" role="option" aria-selected="${selected ? 'true' : 'false'}" data-project-group-chat-option="${escapeAttribute(member.name)}">
            <span class="project-group-chat-check"><i data-lucide="check"></i></span>
            <span class="group-member-avatar ${member.isAgent ? 'agent' : 'human'}">${escapeHtml(member.name.slice(0, 1))}</span>
            <span><strong>${escapeHtml(member.name)}</strong><small>${escapeHtml(member.role)}</small></span>
            <em>${member.isAgent ? '智能体' : '成员'}</em>
          </button>
        `;
  }).join('') : '<p class="project-group-chat-empty">没有找到匹配的项目成员或专家智能体</p>'}
    </div>
  `;
}

function renderProjectGroupChatSelectedPanel(project) {
  const editingGroup = getProjectGroups(project)
    .find((group) => group.id === state.projectGroupChatEditingGroupId);
  const directoryByName = new Map(getProjectGroupChatDirectory(project).map((member) => [member.name, member]));
  const selectedMembers = state.projectGroupChatSelection
    .map((name) => directoryByName.get(name))
    .filter(Boolean);
  return `
    ...
    return `
            <article class="${member.name === 'Snack' ? 'snack' : ''}">
              <span class="group-member-avatar ${member.isAgent ? 'agent' : 'human'}">${member.name === 'Snack' ? '<i data-lucide="sparkles"></i>' : escapeHtml(member.name.slice(0, 1))}</span>
              <div><strong>${escapeHtml(member.name)}${member.isAgent ? '<em>Agent</em>' : ''}</strong><small>${escapeHtml(member.role)}</small></div>
              ${fixed
    ? `<small>${member.name === currentUserName ? '创建者' : '默认加入'}</small>`
    : `<button type="button" data-project-group-chat-option="${escapeAttribute(member.name)}" aria-label="移除 ${escapeAttribute(member.name)}"><i data-lucide="x"></i></button>`}
            </article>
          `;
  }).join('')}
      </div>
...
function refreshProjectGroupChatDirectory() {
  const project = getProjectById(state.projectGroupChatProjectId);
  if (!project) return;
  const filterNode = document.querySelector('.project-group-chat-filters');
  const optionsNode = document.querySelector('.project-group-chat-options');
  if (filterNode instanceof HTMLElement) filterNode.outerHTML = renderProjectGroupChatFilters();
  if (optionsNode instanceof HTMLElement) optionsNode.outerHTML = renderProjectGroupChatOptions(project);
  renderIcons();
}

function refreshProjectGroupChatSelection() {
  const project = getProjectById(state.projectGroupChatProjectId);
  if (!project) return;
  const optionsNode = document.querySelector('.project-group-chat-options');
  const selectedNode = document.querySelector('.project-group-chat-selected');
  if (optionsNode instanceof HTMLElement) optionsNode.outerHTML = renderProjectGroupChatOptions(project);
  if (selectedNode instanceof HTMLElement) selectedNode.outerHTML = renderProjectGroupChatSelectedPanel(project);
  renderIcons();
}
...
function renderProjectGroupChatModal() {
  ...
  const editingGroup = getProjectGroups(project)
    .find((group) => group.id === state.projectGroupChatEditingGroupId);
  return `
    <section class="project-group-chat-backdrop" data-project-group-chat-backdrop role="presentation">
      <section class="project-group-chat-modal" role="dialog" aria-modal="true" aria-labelledby="projectGroupChatTitle">
        ...
        <div class="project-group-chat-body">
          <section class="project-group-chat-directory">
            <label class="project-group-chat-search">
              <i data-lucide="search"></i>
              <input type="search" value="${escapeAttribute(state.projectGroupChatQuery)}" data-project-group-chat-search placeholder="搜索项目成员或专家智能体" autocomplete="off" />
            </label>
            ${renderProjectGroupChatFilters()}
            ${renderProjectGroupChatOptions(project)}
          </section>
          ${renderProjectGroupChatSelectedPanel(project)}
        </div>
      </section>
    </section>
  `;
...
function closeProjectGroupChat() {
...
function toggleProjectGroupChatParticipant(name) {
  if (!name || name === currentUserName || name === 'Snack') return;
  state.projectGroupChatSelection = state.projectGroupChatSelection.includes(name)
    ? state.projectGroupChatSelection.filter((memberName) => memberName !== name)
    : [...state.projectGroupChatSelection, name];
  refreshProjectGroupChatSelection();
}

function syncProjectGroupChatSearchFocus() {
  window.setTimeout(() => {
    const input = document.querySelector('[data-project-group-chat-search]');
    if (!(input instanceof HTMLInputElement)) return;
    input.focus();
    input.setSelectionRange(input.value.length, input.value.length);
  }, 0);
...
function renderProjectGroupMembersModal() {
  const modal = state.projectGroupMembersModal;
  const context = modal ? getProjectGroupContext(modal.projectId, modal.groupId) : null;
  if (!modal || !context) return '';
  const { project, group } = context;
  const directory = new Map(getProjectGroupChatDirectory(project).map((member) => [member.name, member]));
  const members = (group.participants || []).map((name) => directory.get(name) || {
    name,
    role: name === 'Snack' ? 'Snack 是你的工作伙伴' : '项目群成员',
    isAgent: name === 'Snack' || name.endsWith('Agent'),
  });
  return `
    <section class="project-group-members-backdrop" data-project-group-members-backdrop role="presentation">
      <section class="project-group-members-modal" role="dialog" aria-modal="true" aria-labelledby="projectGroupMembersTitle">
        <header>
          <div>
            <h2 id="projectGroupMembersTitle">人员管理</h2>
            <p>管理当前群聊中的用户、Agent 和机器人，历史消息会保留</p>
          </div>
          <span>
            <button class="project-group-members-add" type="button" data-project-group-members-add>
              <i data-lucide="plus"></i><span>添加成员</span>
            </button>
            <button class="project-group-members-close" type="button" data-project-group-members-close aria-label="关闭人员管理">
              <i data-lucide="x"></i>
            </button>
          </span>
        </header>
        <div class="project-group-members-list">
          ${members.map((member) => {
    const isCreator = member.name === currentUserName;
    const isSnack = member.name === 'Snack';
    return `
            <article class="project-group-members-row">
              <span class="group-member-avatar ${member.isAgent ? 'agent' : 'human'}">${isSnack ? '<i data-lucide="sparkles"></i>' : escapeHtml(member.name.slice(0, 1))}</span>
              <span class="project-group-members-copy">
                <strong>${escapeHtml(member.name)}${member.isAgent ? '<em>Agent</em>' : ''}</strong>
                ${member.isAgent && member.role ? `<small>${escapeHtml(member.role)}</small>` : ''}
              </span>
              ${isCreator
    ? '<small class="project-group-member-fixed">创建者</small>'
    : isSnack
      ? '<small class="project-group-member-fixed">默认加入</small>'
      : `
                <button class="project-group-member-remove" type="button" data-project-group-members-remove="${escapeAttribute(member.name)}">
                  <i data-lucide="log-out"></i><span>移除</span>
                </button>
              `}
            </article>
          `;
...
function openProjectGroupMembersModal(projectId, groupId) {
  if (!getProjectGroupContext(projectId, groupId)) return;
  state.projectGroupChatOpen = false;
  state.projectGroupMembersModal = { projectId, groupId };
  state.groupHeaderMenuOpen = false;
  render();
}

function closeProjectGroupMembersModal() {
  state.projectGroupMembersModal = null;
  render();
}

function addProjectGroupMemberFromManager() {
  const modal = state.projectGroupMembersModal;
  if (!modal) return;
  state.projectGroupMembersModal = null;
  openProjectGroupChat(modal.projectId, modal.groupId);
}

function removeProjectGroupMember(name) {
  const modal = state.projectGroupMembersModal;
  const context = modal ? getProjectGroupContext(modal.projectId, modal.groupId) : null;
  if (!context || name === currentUserName || name === 'Snack') return;
  if (!(context.group.participants || []).includes(name)) return;
  context.group.participants = context.group.participants.filter((memberName) => memberName !== name);
  context.group.updatedAt = '刚刚';
  render();
  showToast(`${name} 已移出群聊`);
}
...
function renderProjectGroupActionModal() {
  ...
  return `
    <section class="project-group-action-backdrop" data-project-group-modal-backdrop role="presentation">
      <section class="project-group-action-modal ${modal.type}" role="dialog" aria-modal="true" aria-labelledby="projectGroupActionTitle">
        <header>
          <div>
            <h2 id="projectGroupActionTitle">${isShare ? '分享群组' : isRename ? '重命名' : '删除群聊'}</h2>
            <p>${isShare
    ? '复制链接后，同项目内用户可以通过该链接加入群聊'
    : isRename
      ? '名称不能为空，建议控制在30个中文字符左右'
      : `确定删除群聊「${escapeHtml(group.name)}」吗？删除后该群聊和会话内容都会被删除。`}</p>
          </div>
          <button type="button" data-project-group-modal-close aria-label="关闭"><i data-lucide="x"></i></button>
        </header>
        ${isShare ? `
          <div class="project-group-action-share">
            <input type="text" value="${escapeAttribute(shareUrl)}" readonly data-project-group-share-url aria-label="群聊分享链接" />
            <button class="primary-button" type="button" data-project-group-modal-action="copy-link">复制链接</button>
          </div>
        ` : isRename ? `
          <div class="project-group-action-form">
            <input type="text" value="${escapeAttribute(group.name)}" maxlength="30" data-project-group-rename-input aria-label="群聊名称" />
          </div>
          <footer>
            <button class="secondary-button" type="button" data-project-group-modal-close>取消</button>
            <button class="primary-button" type="button" data-project-group-modal-action="save-name">保存</button>
          </footer>
        ` : `
          <footer>
            <button class="secondary-button" type="button" data-project-group-modal-close>取消</button>
            <button class="primary-button danger" type="button" data-project-group-modal-action="confirm-delete">确认删除</button>
          </footer>
        `}
      </section>
    </section>
...
function closeProjectGroupActionModal() {
  state.projectGroupActionModal = null;
  render();
}
...
function handleProjectGroupModalAction(action) {
  ...
  const { project, group } = context;
  if (action === 'copy-link') {
    const input = document.querySelector('[data-project-group-share-url]');
  ...
  if (action === 'save-name') {
    const input = document.querySelector('[data-project-group-rename-input]');
    ...
    group.name = nextName.slice(0, 30);
    state.projectGroupActionModal = null;
    render();
    showToast('群聊名称已更新');
    return;
  ...
  if (action === 'confirm-delete') {
    ...
    state.projectGroupPage = Math.min(
      state.projectGroupPage,
      Math.max(1, Math.ceil(project.groups.length / projectGroupPageSize)),
    );
    state.projectGroupActionModal = null;
    render();
    showToast('群聊已删除');
...
function runProjectGroupAction(action, groupId) {
  const context = getActiveProjectGroup(groupId);
  if (!context) return;
  const { project, group } = context;
  if (action === 'members') {
    openProjectGroupMembersModal(project.id, group.id);
    return;
  }
...
function toggleProjectCollaboratorManager(type) {
  ...
  if (!refreshProjectCollaboratorsPage()) render();
}
...
function toggleProjectCollaboratorPicker(type) {
  ...
  state.projectCollaboratorQuery = '';
  state.projectCollaboratorPermissionMember = null;
  if (!refreshProjectCollaboratorsPage({ focusSearch: Boolean(state.projectCollaboratorPickerType) })) render();
}

function toggleProjectCollaboratorPermission(name) {
  const project = getActiveProject();
  if (!project || name === currentUserName || !(project.members || []).includes(name)) return;
  state.projectCollaboratorPermissionMember = state.projectCollaboratorPermissionMember === name ? null : name;
  if (!refreshProjectCollaboratorsPage()) render();
}

function setProjectCollaboratorPermission(name, permission) {
  const project = getActiveProject();
  if (!project || name === currentUserName || !(project.members || []).includes(name)) return;
  const nextPermission = permission === 'manage' ? 'manage' : 'use';
  project.memberPermissions = { ...(project.memberPermissions || {}), [name]: nextPermission };
  project.updated = '现在';
  state.projectCollaboratorPermissionMember = null;
  if (!refreshProjectCollaboratorsPage()) render();
  showToast(`${name} 已设为${getProjectMemberPermissionLabel(nextPermission)}`);
}
...
function addProjectCollaborator(name) {
  ...
  if (member.isAgent) {
    ...
    project.memberPermissions = { ...(project.memberPermissions || {}), [member.name]: 'use' };
  }
...
function renderProjectDetailMemberRow(member, options = {}) {
  const canRemove = !options.readOnly && member.name !== currentUserName && member.name !== 'Snack';
  return `
...
function renderBoardWorkspace({ boardLabel, boardContent, project = null }) {
  ...
  return `
    <section class="board-workspace ${activeIssue ? 'issue-open' : ''}">
      ${boardLabel ? renderIssueWorkspaceTabs(boardLabel) : ''}
...
function runGroupHeaderAction(action, projectId, groupId) {
  ...
  state.groupHeaderMenuOpen = false;
  if (action === 'members') {
    openProjectGroupMembersModal(projectId, groupId);
    return;
  }
...
function removeProjectMemberFromProject(name) {
  ...
  if (project.memberPermissions) {
    const remainingPermissions = { ...project.memberPermissions };
    delete remainingPermissions[name];
    project.memberPermissions = remainingPermissions;
  }
  project.updated = '现在';
  state.memberManagerQuery = '';
  state.projectCollaboratorPermissionMember = null;
...
function renderAgents() {
  ...
  return `<section class="agent-employees-page"><div class="agent-employee-grid" role="list">${agents.map(renderAgentCard).join('')}</div></section>`;
}

function renderAgentCard(agent) {
  const icon = {
    Snack: 'sparkles',
    '投放监控 Agent': 'chart-no-axes-combined',
    '研发交付 Agent': 'code-2',
    '用户研究 Agent': 'search',
    '数据分析 Agent': 'chart-pie',
    '会议纪要 Agent': 'notebook-tabs',
  }[agent.name] || 'bot';
  return `
    <article class="agent-employee-card" role="listitem">
      <span class="agent-employee-icon"><i data-lucide="${icon}"></i></span>
      <h2>${escapeHtml(agent.name)}</h2>
      <p title="${escapeAttribute(agent.desc)}">${escapeHtml(agent.desc)}</p>
      <button type="button" data-agent-chat="${escapeAttribute(agent.name)}" aria-label="和 ${escapeAttribute(agent.name)} 对话">发起会话</button>
    </article>
  `;
}
...
const localKnowledgeAudioExtensions = new Set(['mp3', 'wav', 'm4a', 'aac', 'flac', 'ogg', 'opus']);
const localKnowledgeDocumentExtensions = new Set(['ppt', 'pptx', 'pdf', 'jpg', 'jpeg', 'png', 'gif', 'webp', 'html', 'htm', 'md', 'markdown', 'txt', 'doc', 'docx', 'csv', 'xlsx', 'xls']);

function getLocalKnowledgeFileKind(name) {
  const extension = String(name).split('.').pop().toLowerCase();
  if (localKnowledgeAudioExtensions.has(extension)) return 'audio';
  if (localKnowledgeDocumentExtensions.has(extension)) return 'document';
  return 'unsupported';
}

function getLocalKnowledgeSummary(files = state.localKnowledgeFiles) {
  return files.reduce((summary, file) => {
    summary[getLocalKnowledgeFileKind(file.name)] += 1;
    return summary;
  }, { audio: 0, document: 0, unsupported: 0 });
}

function getLocalKnowledgeAssetName() {
  if (state.localKnowledgeSource === 'folder') {
    const relativePath = state.localKnowledgeFiles.find((file) => file.relativePath)?.relativePath || '';
    return relativePath.split('/')[0] || '本地文件夹';
  }
  if (state.localKnowledgeFiles.length === 1) return state.localKnowledgeFiles[0].name;
  return `本地文件 · ${state.localKnowledgeFiles.length} 项`;
}

function renderLocalKnowledgeModal() {
  if (!state.localKnowledgeModalOpen) return '';
  const files = state.localKnowledgeFiles;
  const summary = getLocalKnowledgeSummary(files);
  const acceptedCount = summary.audio + summary.document;
  const canUpload = acceptedCount > 0;
  return `
    <section class="local-knowledge-backdrop" data-local-knowledge-backdrop role="presentation">
      <section class="local-knowledge-modal" role="dialog" aria-modal="true" aria-labelledby="localKnowledgeTitle">
        <header class="local-knowledge-header">
          <div><h2 id="localKnowledgeTitle">已添加到项目资产</h2><p>资产已生成；同步云端、分享和删除可在资产条目旁继续操作。</p></div>
          <button class="local-knowledge-close" type="button" aria-label="关闭本地文件引导" data-close-local-knowledge><i data-lucide="x"></i></button>
        </header>
        <div class="local-knowledge-body">
          ${files.length ? `
            <section class="local-knowledge-scan-result" aria-live="polite">
              <header><span class="local-knowledge-scan-icon"><i data-lucide="${state.localKnowledgeSource === 'folder' ? 'folder-open' : 'files'}"></i></span><div><strong>${escapeHtml(getLocalKnowledgeAssetName())}</strong><small>已检测 ${files.length} 项内容${summary.unsupported ? `，${summary.unsupported} 项暂不支持` : ''}</small></div></header>
              <div class="local-knowledge-type-grid">
                <span><i data-lucide="audio-lines"></i><strong>${summary.audio}</strong> 个音频</span>
                <span><i data-lucide="file-text"></i><strong>${summary.document}</strong> 个其他文件</span>
                ${summary.unsupported ? `<span class="unsupported"><i data-lucide="circle-alert"></i><strong>${summary.unsupported}</strong> 个跳过</span>` : ''}
              </div>
              ${summary.audio ? `<p><i data-lucide="badge-check"></i>音频上传后将转码并生成可检索文本。</p>` : ''}
              ${summary.document ? `<p><i data-lucide="badge-check"></i>文档、图片和表格将按原文件类型读取。</p>` : ''}
            </section>
          ` : `<div class="local-knowledge-empty"><i data-lucide="folder-search-2"></i><strong>还没有选择内容</strong><span>选择后会检测文件类型，并展示对应的上传处理方式。</span></div>`}
        </div>
        <footer class="local-knowledge-footer"><span>${files.length ? `已生成 1 个${state.localKnowledgeSource === 'folder' ? '文件夹' : '本地文件'}资产条目` : ''}</span><div><button class="primary-button" type="button" data-submit-local-knowledge ${canUpload ? '' : 'disabled'}><i data-lucide="check"></i>完成</button></div></footer>
      </section>
    </section>
  `;
}

function renderLocalKnowledgePickerInputs() {
  return `<input id="localKnowledgeFilesInput" data-local-knowledge-input="files" type="file" multiple hidden accept=".mp3,.wav,.m4a,.aac,.flac,.ogg,.opus,.ppt,.pptx,.pdf,.jpg,.jpeg,.png,.gif,.webp,.html,.htm,.md,.markdown,.txt,.doc,.docx,.csv,.xlsx,.xls" />
    <input id="localKnowledgeFolderInput" data-local-knowledge-input="folder" type="file" webkitdirectory directory multiple hidden />`;
}

function openLocalKnowledgeModal(projectId) {
  openLocalKnowledgePicker(projectId);
}

function openLocalKnowledgePicker(projectId) {
  if (!getProjectById(projectId)) return;
  state.localKnowledgeModalOpen = false;
  state.localKnowledgeProjectId = projectId;
  state.localKnowledgeAssetId = null;
  state.localKnowledgeFiles = [];
  state.localKnowledgeSource = null;
  state.localKnowledgeSyncToCloud = true;
  state.localKnowledgeShareEnabled = false;
  state.localKnowledgeShareProjectIds = [];
  render();
  window.setTimeout(() => {
    const input = document.querySelector('#localKnowledgeFilesInput');
    if (input instanceof HTMLInputElement) input.click();
  }, 0);
}

function closeLocalKnowledgeModal() {
  state.localKnowledgeModalOpen = false;
  state.localKnowledgeProjectId = null;
  render();
}

function handleLocalKnowledgeSelection(input) {
  const source = input.dataset.localKnowledgeInput;
  state.localKnowledgeFiles = [...input.files].map((file) => ({ name: file.name, relativePath: file.webkitRelativePath || '' }));
  state.localKnowledgeSource = source === 'folder' ? 'folder' : 'files';
  createLocalKnowledgeAsset();
}

function createLocalKnowledgeAsset() {
  const files = state.localKnowledgeFiles.filter((file) => getLocalKnowledgeFileKind(file.name) !== 'unsupported');
  if (!files.length) return showToast('请选择支持的本地文件');
  const summary = getLocalKnowledgeSummary(files);
  const project = getProjectById(state.localKnowledgeProjectId);
  if (!project) return closeLocalKnowledgeModal();
  const contentSummary = [summary.audio ? `${summary.audio} 个音频（将转码）` : '', summary.document ? `${summary.document} 个文件` : ''].filter(Boolean).join('、');
  const asset = {
    ...
    label: `${contentSummary} · 仅本地读取`, updated: '刚刚', updatedBy: currentUserName,
    visibility: 'private', cloudSynced: false, sharedProjectIds: [],
  };
  if (state.projectCreationOpen && state.projectEditingId === project.id) {
    state.projectCreationFolders = [...(state.projectCreationFolders || []), asset];
  } else {
  ...
  state.localKnowledgeModalOpen = false;
  render();
  showToast(`本地内容已添加到“${project.title}”的项目资产`);
}

function submitLocalKnowledge() {
  state.localKnowledgeModalOpen = false;
  state.localKnowledgeProjectId = null;
  state.localKnowledgeAssetId = null;
  state.localKnowledgeFiles = [];
...
function getProjectLocalAsset(projectId, assetId) {
  return ensureProjectSourceAssets(getProjectById(projectId)).find((item) => item.id === assetId) || null;
}
...
function runProjectAssetAction(action, projectId, assetId) {
  ...
  if (action === 'cloud') {
    ...
    asset.label = asset.label.replace('仅本地读取', '已同步到云端');
    render();
    showToast('该资产已同步到云端');
    return;
  }
  ...
  if (action === 'transcribe') {
    ...
    asset.label = `${asset.contentSummary || asset.label.replace(/ · .+$/, '')} · 已转写${asset.cloudSynced ? ' · 已同步到云端' : ' · 仅本地读取'}`;
    render();
    showToast('音频已转写完成，可粘贴到会议输入框生成纪要');
    return;
  }
  if (action === 'meeting-input') {
    if (asset.kind !== 'local-files') return;
    state.projectChatDraftProjectId = projectId;
    state.projectChatDraft = `请基于以下音频转写生成会议纪要：\n\n【${asset.name}】\n（已完成 ${asset.audioCount || 1} 段音频转写，可在此继续补充会议背景和参会人。）`;
    openProjectSession(projectId, 'group');
    showToast('已前往会议纪要生成，可继续补充会议信息');
    return;
  }
  if (action === 'delete') {
...
function getResourceIcon(item) {
  if (item.source === 'local') return item.meta?.startsWith('本地文件夹') ? 'folder-open' : 'file-stack';
...
function renderProjectMembersModule() {
  const selectedCount = (state.projectCreationMembers || []).length + 1;
  const editorOpen = state.projectMemberEditorOpen;
  return `
    <section class="project-edit-module project-edit-members ${editorOpen ? 'expanded' : ''}" data-project-members-module>
      <header class="project-edit-module-header">
        <div class="project-edit-module-title">
          <span><i data-lucide="users-round"></i></span>
          <div>
            <strong>成员 <em>可选</em></strong>
            <small>项目成员可以共享项目内容，快速发起群聊</small>
          </div>
        </div>
        <span class="project-edit-module-actions">
          <em>${selectedCount} 位</em>
          <button type="button" data-project-members-add aria-expanded="${editorOpen ? 'true' : 'false'}">
            <span>${editorOpen ? '收起' : '展开'}</span>
            <i data-lucide="${editorOpen ? 'chevron-up' : 'chevron-down'}"></i>
          </button>
        </span>
      </header>
      ${editorOpen ? `
        <div class="project-edit-module-body project-edit-members-body">
...
function toggleProjectMembersEditor() {
  state.projectMemberEditorOpen = !state.projectMemberEditorOpen;
...
function renderProjectKnowledgeModule() {
  const topics = state.projectCreationWikiTopics || [];
  const folders = state.projectCreationFolders || [];
  const sessionAssets = state.projectCreationSessionAssets || [];
  const resourceCount = topics.length + folders.length + sessionAssets.length;
  const editorOpen = state.projectKnowledgeEditorOpen;
  const editingProject = getProjectById(state.projectEditingId);
  const projectId = editingProject?.id || '';
  return `
    <section class="project-edit-module project-edit-resources ${editorOpen ? 'expanded' : ''}" data-project-knowledge-module>
      <header class="project-edit-module-header">
        <div class="project-edit-module-title">
          <span><i data-lucide="paperclip"></i></span>
          <div>
            <strong>关联资产 <em>可选</em></strong>
            <small>连接语雀空间、LLM Wiki 或本地项目文件</small>
          </div>
        </div>
        <span class="project-edit-module-actions">
          ${resourceCount ? `<em>${resourceCount} 项</em>` : ''}
          <button type="button" data-project-knowledge-add aria-expanded="${editorOpen ? 'true' : 'false'}">
            <span>${editorOpen ? '收起' : '展开'}</span>
            <i data-lucide="${editorOpen ? 'chevron-up' : 'chevron-down'}"></i>
          </button>
        </span>
      ...
      ${editorOpen ? `
        <div class="project-edit-module-body project-edit-resource-body">
          <div class="project-config-asset-actions" aria-label="添加项目资产">
            ${renderProjectWikiTopicPicker()}
            <button type="button" ${projectId ? `data-project-session-import-open="${escapeAttribute(projectId)}"` : 'disabled title="保存项目后可导入会话"'}>
              <i data-lucide="messages-square"></i><span>导入会话</span>
            </button>
            <button type="button" ${projectId ? `data-open-local-knowledge="${escapeAttribute(projectId)}"` : 'disabled title="保存项目后可添加本地文件"'}>
              <i data-lucide="folder-up"></i><span>添加本地文件</span>
            </button>
          </div>
          ${resourceCount ? `
            <div class="project-config-asset-selection" aria-label="已关联资产">
              ${topics.map((topicName) => `
                <span>
                  <strong>${escapeHtml(topicName)}</strong><small>知识</small>
                  <button type="button" aria-label="移除 ${escapeAttribute(topicName)}" data-project-config-asset-remove="wiki-topic" data-project-config-asset-id="${escapeAttribute(topicName)}"><i data-lucide="x"></i></button>
                </span>
              `).join('')}
              ${sessionAssets.map((session) => `
                <span>
                  <strong>${escapeHtml(session.name)}</strong><small>会话</small>
                  <button type="button" aria-label="移除 ${escapeAttribute(session.name)}" data-project-config-asset-remove="session" data-project-config-asset-id="${escapeAttribute(session.id)}"><i data-lucide="x"></i></button>
                </span>
              `).join('')}
              ${folders.map((folder) => `
                <span>
                  <strong>${escapeHtml(folder.name)}</strong><small>本地文件</small>
                  <button type="button" aria-label="移除 ${escapeAttribute(folder.name)}" data-project-config-asset-remove="folder" data-project-config-asset-id="${escapeAttribute(folder.name)}"><i data-lucide="x"></i></button>
                </span>
              `).join('')}
            </div>
          ` : '<p class="project-config-asset-empty">尚未关联资产，可从上方选择一种方式添加。</p>'}
        </div>
      ` : ''}
    </section>
  `;
...
function renderProjectLocalWorkspaceModule() {
  const desktopRuntime = isSnackDesktopRuntime();
  const promptOpen = state.projectLocalWorkspacePromptOpen;
  return `
    <section class="project-edit-module project-edit-local-workspace ${desktopRuntime && promptOpen ? 'expanded' : ''}" data-project-local-workspace-module>
      <header class="project-edit-module-header">
        <div class="project-edit-module-title">
          <span><i data-lucide="hard-drive"></i></span>
          <div>
            <strong>关联本地工作区 <em>可选</em></strong>
            <small>让 Snack 桌面端读取和编辑指定的本地项目目录</small>
          </div>
        </div>
        <span class="project-edit-module-actions">
          ${desktopRuntime ? `
            <button type="button" data-project-local-workspace-toggle aria-expanded="${promptOpen}">
              <span>${promptOpen ? '收起' : '展开'}</span><i data-lucide="chevron-${promptOpen ? 'up' : 'down'}"></i>
            </button>
          ` : `
            <button class="project-local-workspace-download" type="button" data-snack-desktop-download>
              <i data-lucide="download"></i><span>下载桌面端</span>
            </button>
          `}
        </span>
      </header>
      ${desktopRuntime && promptOpen ? `
        <div class="project-edit-module-body project-local-workspace-body">
          <div class="project-local-workspace-picker">
            <span>
              <strong>尚未关联本地工作区</strong>
              <small>选择一个本地目录，Snack 将在授权范围内读取和编辑内容</small>
            </span>
            <button type="button" data-project-local-workspace-select>
              <i data-lucide="folder-open"></i><span>选择工作区</span>
            </button>
          </div>
        </div>
      ` : ''}
...
function toggleProjectLocalWorkspacePrompt() {
  if (!isSnackDesktopRuntime()) return;
  state.projectLocalWorkspacePromptOpen = !state.projectLocalWorkspacePromptOpen;
  const module = document.querySelector('[data-project-local-workspace-module]');
  if (!(module instanceof HTMLElement)) {
    render();
    return;
  }
  module.outerHTML = renderProjectLocalWorkspaceModule();
  renderIcons();
}

function refreshProjectKnowledgeModule() {
  const module = document.querySelector('[data-project-knowledge-module]');
  if (!(module instanceof HTMLElement)) return false;
  module.outerHTML = renderProjectKnowledgeModule();
  renderIcons();
  return true;
}

function toggleProjectKnowledgeEditor() {
  state.projectKnowledgeEditorOpen = !state.projectKnowledgeEditorOpen;
  state.projectWikiTopicPickerOpen = false;
  if (state.projectMemberPickerOpen) {
    state.projectMemberPickerOpen = false;
    refreshProjectMemberPicker();
  }
  refreshProjectKnowledgeModule();
}
...
function openProjectCreationModal(projectId = null, options = {}) {
  ...
  state.projectMemberQuery = '';
  state.projectMemberPickerOpen = false;
  state.projectMemberPickerType = 'human';
  state.projectMemberEditorOpen = Boolean(editingProject && state.projectCreationMembers.length);
  state.openProjectMenuId = null;
  state.openProjectCreateMenuId = null;
  state.modelPickerOpen = false;
  state.projectPickerOpen = false;
...
function closeProjectCreationModal() {
  ...
  state.projectMemberPickerOpen = false;
  state.projectMemberPickerType = 'human';
  state.projectMemberEditorOpen = false;
  render();
}
...
function renderProjectWikiTopicPicker() {
  ...
  return `
    <span class="project-wiki-topic-picker project-config-asset-action-wrap ${state.projectWikiTopicPickerOpen ? 'open' : ''}" data-project-wiki-topic-picker>
      <button
        class="project-config-asset-action"
        type="button"
        ...
        aria-label="添加知识"
      >
        <i data-lucide="plus"></i><span>添加知识</span>
      </button>
      ${state.projectWikiTopicPickerOpen ? `
        <section class="project-wiki-topic-options" id="projectWikiTopicOptions" role="listbox" aria-label="可关联的云端资产" aria-multiselectable="true">
          ${projectWikiTopicOptions.map((topic) => {
    ...
    return `
              ...
              >
                <span class="project-wiki-topic-copy">
                  ...
                  <small><em>${escapeHtml(topic.source)}</em>${escapeHtml(topic.description)}</small>
                </span>
                <span class="project-wiki-topic-check" aria-hidden="true"><i data-lucide="check"></i></span>
              </button>
            `;
  }).join('')}
          <footer>${selectedTopics.length ? `已选择 ${selectedTopics.length} 项云端资产` : '支持同时选择多个云端资产'}</footer>
        </section>
      ` : ''}
    </span>
  `;
...
function toggleProjectWikiTopic(topicName) {
  ...
  if (!refreshProjectKnowledgeModule()) refreshProjectWikiTopicPicker();
}
...
function removeProjectConfigAsset(kind, assetId) {
  if (kind === 'wiki-topic') {
    state.projectCreationWikiTopics = (state.projectCreationWikiTopics || []).filter((topicName) => topicName !== assetId);
  } else if (kind === 'session') {
    state.projectCreationSessionAssets = (state.projectCreationSessionAssets || []).filter((session) => session.id !== assetId);
  } else if (kind === 'folder') {
    state.projectCreationFolders = (state.projectCreationFolders || []).filter((folder) => folder.name !== assetId);
  } else {
    return;
  }
  refreshProjectKnowledgeModule();
...
function renderProjectMemberPicker() {
  ...
  const options = getProjectMemberDirectory()
    .filter((member) => !selectedNames.has(member.name))
    .filter((member) => !grouped || member.isAgent === (activeType === 'agent'))
    .filter((member) => !query || `${member.name} ${member.role} ${member.isAgent ? 'agent 智能体' : '人员 同事'}`.toLowerCase().includes(query));
  if (grouped) {
    const selectedPeople = [
      { name: currentUserName, isAgent: false, role: '项目发起人', locked: true },
      ...selectedMembers.filter((member) => !member.isAgent),
    ];
    const selectedAgents = selectedMembers.filter((member) => member.isAgent);
    return `
      <div class="project-member-picker project-member-source-list ${state.projectMemberPickerOpen ? 'open' : ''}" data-project-member-picker>
          ${renderProjectMemberSource(
    'human',
    '项目成员',
    '选择可以查看项目内容并参与群聊的成员',
    selectedPeople,
    state.projectMemberPickerOpen && activeType === 'human' ? options : null,
  )}
          ${renderProjectMemberSource(
    'agent',
    '专家智能体',
    '选择 Snack 现有的智能体员工参与项目',
    selectedAgents,
    state.projectMemberPickerOpen && activeType === 'agent' ? options : null,
  )}
      </div>
    `;
  }
...
function renderProjectMemberSource(type, title, description, members, options = null) {
  const pickerOpen = Array.isArray(options);
  const pickerLabel = type === 'agent' ? '专家智能体' : '项目成员';
  const queryValue = pickerOpen ? state.projectMemberQuery : '';
  return `
    <section class="project-member-source ${type}">
      <header>
        <span><i data-lucide="${type === 'agent' ? 'bot' : 'user-round'}"></i></span>
        <div>
          <strong>${escapeHtml(title)}</strong>
          <small>${escapeHtml(description)}</small>
        </div>
      </header>
      <div class="project-member-source-control ${pickerOpen ? 'open' : ''}">
        ${members.map((member) => `
          <span class="project-member-selected ${member.isAgent ? 'agent' : 'human'}">
            <em>${escapeHtml(member.name.slice(0, 1))}</em>
            <strong>${escapeHtml(member.name)}</strong>
        ...
        `).join('')}
        <label class="project-member-source-query">
          <i data-lucide="search"></i>
          <input
            type="search"
            data-project-member-search="${type}"
            value="${escapeAttribute(queryValue)}"
            autocomplete="off"
            aria-label="搜索${pickerLabel}"
            aria-expanded="${pickerOpen ? 'true' : 'false'}"
            aria-controls="projectMemberOptions"
            placeholder="搜索并添加"
          />
        </label>
        <i class="project-member-source-chevron" data-lucide="chevron-down"></i>
      </div>
      ${pickerOpen ? `
        <section class="project-member-inline-picker" id="projectMemberOptions" role="listbox" aria-label="可添加的${pickerLabel}">
          <div class="project-member-option-list">
            ${options.length ? options.map(renderProjectMemberOption).join('') : `
              <p class="project-member-empty">没有找到可添加的${pickerLabel}</p>
            `}
          </div>
        </section>
      ` : ''}
...
function renderProjectMemberOption(member) {
  return `
    <button class="project-member-option" type="button" role="option" data-project-member-option="${escapeAttribute(member.name)}">
      ...
      <em>${member.isAgent ? '智能体员工' : '项目成员'}</em>
    </button>
  `;
}
...
function syncProjectMemberSearchFocus() {
  window.setTimeout(() => {
    const activeType = state.projectMemberPickerType === 'agent' ? 'agent' : 'human';
    const input = document.querySelector(`[data-project-member-search="${activeType}"]`)
      || document.querySelector('[data-project-member-search]');
    if (!(input instanceof HTMLInputElement)) return;
    input.focus();
    input.setSelectionRange(input.value.length, input.value.length);
  }, 0);
...
function toggleProjectMemberPicker(type) {
  const normalizedType = type === 'agent' ? 'agent' : 'human';
  if (state.projectMemberPickerOpen && state.projectMemberPickerType === normalizedType) {
    closeProjectMemberPicker();
    return;
  }
  openProjectMemberPicker(normalizedType);
}

function openProjectMemberPicker(type = state.projectMemberPickerType) {
  if (state.projectWikiTopicPickerOpen) {
    state.projectWikiTopicPickerOpen = false;
    refreshProjectWikiTopicPicker();
  }
  state.projectMemberPickerType = type === 'agent' ? 'agent' : 'human';
  state.projectMemberQuery = '';
...
function createNewProject(form) {
  ...
  const sessionAssets = (state.projectCreationSessionAssets || []).map((session) => ({ ...session }));
  ...
  const hasMonitoringEditor = Boolean(form.querySelector('[data-monitoring-rules-editor]'));
  const monitoringDraft = hasMonitoringEditor ? String(formData.get('monitoring-rule-draft') || '').trim() : '';
  const monitoringRules = hasMonitoringEditor
    ? parseProjectMonitoringRules(formData)
    : (editingProject?.monitoringRules || []).map((rule) => ({ ...rule }));
  ...
  if (editingProject) {
    const previousRememberedPeople = new Map(editingProject.rememberedPeople || []);
    ...
    editingProject.members = [currentUserName, ...invitedPeople.map((member) => member.name)];
    editingProject.agents = ['Snack', ...invitedAgents.map((member) => member.name)];
    editingProject.rememberedPeople = [
      [currentUserName, previousRememberedPeople.get(currentUserName) || '项目发起人，负责目标确认和执行授权'],
      ...invitedPeople.map((member) => [
        member.name,
        previousRememberedPeople.get(member.name) || '受邀项目成员，参与项目协作和任务确认',
      ]),
      ...invitedAgents.map((member) => [
        member.name,
        previousRememberedPeople.get(member.name) || `受邀 ${member.role}，按项目任务节点处理对应工作`,
      ]),
    ];
    editingProject.wikiTopics = wikiTopics;
    editingProject.sessionAssets = sessionAssets;
  ...
  project.sessionAssets = sessionAssets;
  ...
  state.projectBoardTab = 'tasks';
  state.projectDetailTab = 'collaborators';
  state.activeProject = projectId;
  state.activeSession = null;
  state.activeIssue = null;
  state.activeIssueTab = null;
...
function runProjectMenuAction(action, projectId) {
  if (action === 'add-group') return openProjectGroupChat(projectId);
...
function createGroupChat(projectId, participants = null) {
  ...
  if (Array.isArray(participants)) {
    const invitedNames = participants.filter((name) => name !== currentUserName && name !== 'Snack');
    project.activeGroupParticipants = [...new Set([currentUserName, 'Snack', ...invitedNames])];
    project.groupMessages.push(
      [currentUserName, invitedNames.length ? `发起群聊，邀请 ${invitedNames.join('、')} 加入。` : '发起一个新的项目群聊。', '刚刚'],
      ['Snack', '我已加入群聊，会结合项目目标、任务和关联知识协助大家推进。', '刚刚'],
    );
  }
...
function handleBodyClick(event) {
  ...
  const clickedFloatingControl = target.dataset.modelMenu
    ...
    || target.dataset.projectKnowledgePickerToggle
    || target.dataset.projectKnowledgeTopic
    || (target.dataset.projectWikiTopicCreate !== undefined)
    || (target.dataset.projectKnowledgeAdd !== undefined)
    || (target.dataset.projectMembersAdd !== undefined)
    || (target.dataset.projectMemberPicker !== undefined)
    || target.dataset.projectMemberAdd
  ...
  const clickedProjectPermissionControl = target.dataset.projectCollaboratorPermission !== undefined
    || target.dataset.projectCollaboratorPermissionValue !== undefined;
  if (state.projectCollaboratorPermissionMember && !clickedProjectPermissionControl) {
    state.projectCollaboratorPermissionMember = null;
    document.querySelector('.project-collaborator-permission-menu')?.remove();
    const permissionWrap = document.querySelector('.project-collaborator-permission.open');
    if (permissionWrap instanceof HTMLElement) {
      permissionWrap.classList.remove('open');
      permissionWrap.querySelector('[aria-expanded="true"]')?.setAttribute('aria-expanded', 'false');
    }
  }
  ...
  if (target.dataset.projectGroupModalAction) return handleProjectGroupModalAction(target.dataset.projectGroupModalAction);
  if (target.dataset.projectGroupMembersClose !== undefined) return closeProjectGroupMembersModal();
  if (target.dataset.projectGroupMembersBackdrop !== undefined) {
    if (event.target === target) closeProjectGroupMembersModal();
    return;
  }
  if (target.dataset.projectGroupMembersAdd !== undefined) return addProjectGroupMemberFromManager();
  if (target.dataset.projectGroupMembersRemove) return removeProjectGroupMember(target.dataset.projectGroupMembersRemove);
  if (target.dataset.localKnowledgeBackdrop !== undefined) {
    if (event.target === target) closeLocalKnowledgeModal();
    return;
  }
  ...
  if (target.dataset.projectGroupChatClose !== undefined) return closeProjectGroupChat();
  if (target.dataset.projectGroupChatBackdrop !== undefined) {
    if (event.target === target) closeProjectGroupChat();
    return;
  }
  ...
  if (target.dataset.projectCollaboratorPermissionValue) {
    return setProjectCollaboratorPermission(
      target.dataset.projectCollaboratorPermissionMember,
      target.dataset.projectCollaboratorPermissionValue,
    );
  }
  if (target.dataset.projectCollaboratorPermission) return toggleProjectCollaboratorPermission(target.dataset.projectCollaboratorPermission);
  ...
  if (target.dataset.projectWikiTopicCreate !== undefined) return simulateWikiTopicCreationLink();
  ...
  if (target.dataset.projectConfigAssetRemove) return removeProjectConfigAsset(target.dataset.projectConfigAssetRemove, target.dataset.projectConfigAssetId);
  ...
  if (target.dataset.projectKnowledgeAdd !== undefined) return toggleProjectKnowledgeEditor();
  if (target.dataset.projectLocalWorkspaceToggle !== undefined) return toggleProjectLocalWorkspacePrompt();
  if (target.dataset.projectLocalWorkspaceSelect !== undefined) return showToast('已打开本地工作区选择器');
  if (target.dataset.projectMembersAdd !== undefined) return toggleProjectMembersEditor();
  if (target.dataset.projectMemberAdd) return toggleProjectMemberPicker(target.dataset.projectMemberAdd);
  ...
  if (target.dataset.projectMemberSearch !== undefined || target.dataset.projectMemberPicker !== undefined) {
    const requestedType = target.dataset.projectMemberSearch;
    if (!state.projectMemberPickerOpen || (requestedType && state.projectMemberPickerType !== requestedType)) {
      openProjectMemberPicker(requestedType || state.projectMemberPickerType);
    }
    return;
  }
  if (target.dataset.snackDesktopDownload !== undefined) return showToast('请前往 Snack 官网下载桌面端');
...
document.body.addEventListener('input', (event) => {
  ...
  if (event.target instanceof HTMLInputElement && event.target.dataset.projectCreateName !== undefined) {
    const counter = event.target.closest('.project-edit-name-field')?.querySelector('[data-project-name-count]');
    if (counter instanceof HTMLElement) counter.textContent = `${event.target.value.length}/40`;
    return;
  }
  ...
  if (event.target instanceof HTMLInputElement && event.target.dataset.projectMemberSearch !== undefined) {
    const requestedType = event.target.dataset.projectMemberSearch;
    if (requestedType) state.projectMemberPickerType = requestedType === 'agent' ? 'agent' : 'human';
...
document.body.addEventListener('change', (event) => {
  ...
  if (target instanceof HTMLInputElement && target.dataset.projectAssetSelectAll !== undefined) {
    ...
    const assetIds = [
      ...
      ...ensureProjectSourceAssets(project).map((folder) => folder.id),
    ];
  ...
  if (target instanceof HTMLInputElement && target.dataset.projectAssetShareProject) {
    ...
    const projectId = target.dataset.projectAssetShareProject;
  ...
  if (target instanceof HTMLInputElement && target.dataset.localKnowledgeInput !== undefined) {
    handleLocalKnowledgeSelection(target);
    return;
  }
  if (target instanceof HTMLInputElement && target.dataset.localKnowledgeSync !== undefined) {
    state.localKnowledgeSyncToCloud = target.checked;
    return;
  }
  if (target instanceof HTMLInputElement && target.dataset.localKnowledgeShareToggle !== undefined) {
    state.localKnowledgeShareEnabled = target.checked;
    if (!target.checked) state.localKnowledgeShareProjectIds = [];
    render();
    return;
  }
  if (target instanceof HTMLInputElement && target.dataset.localKnowledgeProject) {
    state.localKnowledgeShareProjectIds = target.checked
      ? [...new Set([...state.localKnowledgeShareProjectIds, target.dataset.localKnowledgeProject])]
      : state.localKnowledgeShareProjectIds.filter((id) => id !== target.dataset.localKnowledgeProject);
    return;
  }
...
document.body.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'Escape' && state.projectCollaboratorPermissionMember) {
    event.preventDefault();
    state.projectCollaboratorPermissionMember = null;
    if (!refreshProjectCollaboratorsPage()) render();
    return;
